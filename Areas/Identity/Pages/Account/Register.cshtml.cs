...
namespace NET_Advanced.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private NET_AdvancedUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<NET_AdvancedUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(NET_AdvancedUser)}'. " +
                    $"Ensure that '{nameof(NET_AdvancedUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<NET_AdvancedUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<NET_AdvancedUser>)_userStore;
        }
    }
}
